...
export const sendPasswordResetOTP = async (email, otp) => {
	...
	try {
		await sendBrevoEmail(
			email,
			"Password Reset OTP - Biruh Kids Clinic",
			htmlContent
		);
		return { success: true };
	} catch (error) {
...
export const sendResultNotification = (email, name, resultFiles = []) => {
	setImmediate(async () => {
		...
		try {
			await sendBrevoEmail(
				email,
				"Test Results Available - Biruh Kids Clinic",
				htmlContent,
				"Biruh Kids Pediatric Speciality Clinic"
			);
			console.log(`Result notification sent to ${email}`);
		} catch (error) {
			console.error(
				`Failed to send result notification to ${email}:`,
				error
			);
		}
	});
...
// Send subscription confirmation email (REMOVED - no longer used)
// This function is kept for backward compatibility but should not be called
export const sendSubscriptionEmail = async (email) => {
	console.log(`Subscription email function called but disabled for: ${email}`);
	return { success: true, message: "Subscription email disabled" };
};
...
export const sendNewDoctorNewsletter = async (doctor) => {
	try {
		...
		console.log(
			`Found ${uniqueRecipients.length} unique recipients for doctor newsletter`
		);
		...
		for (let i = 0; i < uniqueRecipients.length; i += batchSize) {
			...
			const emailPromises = batch.map(async (recipient) => {
				// Check if recipient is a newsletter subscriber
				const isNewsletterSubscriber = activeSubscribers.some(sub => sub.email === recipient.email);
				const unsubscribeUrl = `${
					process.env.BACKEND_URL || "http://localhost:5000"
				}/subscribers/unsubscribe/${encodeURIComponent(recipient.email)}`;
                        ...
                        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; background: #f8f9fa; padding: 20px;">
                            <div style="background: white; padding: 30px; border-radius: 10px;">
                                ...
                                <div style="text-align: center; margin: 20px 0;">
                                    ${
										doctor.photo
											? `<img src="${doctor.photo}" alt="Dr. ${doctor.name}" style="width: 150px; height: 150px; border-radius: 50%; object-fit: cover; border: 3px solid #007799; display: block; margin: 0 auto;">`
											: `<div style="width: 150px; height: 150px; border-radius: 50%; background: #f0f0f0; display: flex; align-items: center; justify-content: center; margin: 0 auto; border: 3px solid #007799;"><span style="color: #666; font-size: 14px;">No Photo</span></div>`
									}
                                </div>
                                <h3 style="color: #333; text-align: center;">Dr. ${
									doctor.name
								}</h3>
                                <div style="background: #e8f4f8; padding: 15px; border-radius: 5px; margin: 15px 0;">
                                    <p><strong>Specialty:</strong> ${
										doctor.field || "Not specified"
									}</p>
                                    <p><strong>Experience:</strong> ${
										doctor.experience || "Not specified"
									}</p>
                                </div>
                                <p style="text-align: center; color: #666;">We're excited to welcome Dr. ${
									doctor.name
								} to our team</p>
                                <div style="text-align: center; margin: 30px 0;">
                                    <a href="${unsubscribeUrl}" style="background-color: #dc2626; color: white; padding: 8px 16px; text-decoration: none; border-radius: 4px; font-size: 12px;">Unsubscribe</a>
                                </div>
                                <hr style="margin: 30px 0; border: none; border-top: 1px solid #eee;">
				...
				try {
					await sendBrevoEmail(
						recipient.email,
						"New Doctor Joined Our Team! - Biruh Kids Clinic",
						htmlContent,
						"Biruh Kids Pediatric Speciality Clinic"
					);
					...
					console.error(
						`Failed to send newsletter to ${recipient.email}:`,
						error
					);
					return false;
				}
		...
		console.log(
			`Newsletter batch processing completed: ${sentCount}/${uniqueRecipients.length} emails sent`
		);
		return {
			success: true,
			sent: sentCount,
...
export const sendNewServiceNewsletter = (service) => {
	setImmediate(async () => {
		...
		for (let i = 0; i < uniqueRecipients.length; i += batchSize) {
			...
			const emailPromises = batch.map(async (recipient) => {
                            ...
                            <div style="background: white; padding: 30px; border-radius: 10px;">
                                ...
                                <div style="text-align: center; margin: 25px 0;">
                                    <a href="${
										...
										"http://localhost:5173"
									}/services" style="background: #007799; color: white; padding: 12px 25px; text-decoration: none; border-radius: 5px;">View All Services</a>
                                </div>
                                <div style="text-align: center; margin: 20px 0;">
                                    <a href="${unsubscribeUrl}" style="background-color: #dc2626; color: white; padding: 8px 16px; text-decoration: none; border-radius: 4px; font-size: 12px;">Unsubscribe</a>
                                </div>
				...
				try {
					await sendBrevoEmail(
						recipient.email,
						"New Service Available! - Biruh Kids Clinic",
						htmlContent,
						"Biruh Kids Pediatric Speciality Clinic"
					);
					return true;
					...
					console.error(
						`Failed to send service newsletter to ${recipient.email}:`,
						error
					);
					return false;
				}
		...
		console.log(
			`Service newsletter sent to ${sentCount}/${uniqueRecipients.length} recipients`
		);
	});
};
...
export const sendNewVideoNewsletter = (video) => {
	setImmediate(async () => {
		...
		for (let i = 0; i < uniqueRecipients.length; i += batchSize) {
			...
			const emailPromises = batch.map(async (recipient) => {
                        ...
                        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; background: #f8f9fa; padding: 20px;">
                            <div style="background: white; padding: 30px; border-radius: 10px;">
                                ...
                                <div style="text-align: center; margin: 25px 0;">
                                    <a href="${
										video.url ||
										process.env.FRONTEND_URL + "/videos"
									}" style="background: #007799; color: white; padding: 12px 25px; text-decoration: none; border-radius: 5px;">Watch on ${
				...
				try {
					await sendBrevoEmail(
						recipient.email,
						"New Educational Video Available! - Biruh Kids Clinic",
						htmlContent,
						"Biruh Kids Pediatric Speciality Clinic"
					);
					return true;
					...
					console.error(
						`Failed to send video newsletter to ${recipient.email}:`,
						error
					);
					return false;
				}
		...
		console.log(
			`Video newsletter sent to ${sentCount}/${uniqueRecipients.length} recipients`
		);
	});
};
...
export const sendNewDepartmentNewsletter = (department) => {
	setImmediate(async () => {
		...
		for (let i = 0; i < uniqueRecipients.length; i += batchSize) {
			...
			const emailPromises = batch.map(async (recipient) => {
				...
				try {
					await sendBrevoEmail(
						recipient.email,
						"New Department Available! - Biruh Kids Clinic",
						htmlContent,
						"Biruh Kids Pediatric Speciality Clinic"
					);
					return true;
					...
					console.error(
						`Failed to send department newsletter to ${recipient.email}:`,
						error
					);
					return false;
				}
		...
		console.log(
			`Department newsletter sent to ${sentCount}/${uniqueRecipients.length} recipients`
		);
	});
};
...
export const sendNewDepartmentServiceNewsletter = (department, service) => {
	setImmediate(async () => {
		...
		for (let i = 0; i < uniqueRecipients.length; i += batchSize) {
			...
			const emailPromises = batch.map(async (recipient) => {
                        ...
                        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; background: #f8f9fa; padding: 20px;">
                            <div style="background: white; padding: 30px; border-radius: 10px;">
                                ...
                                <div style="background: #e8f4f8; padding: 15px; border-radius: 5px; margin: 15px 0;">
                                    ...
                                    ${
										service.name_am
											? `<p><strong>አማርኛ:</strong> ${service.name_am}</p>`
											: ""
									}
                                </div>
                                <p style="text-align: center; color: #666;">We've added a new service to our ${
									department.title_en
								} department!</p>
                                <div style="text-align: center; margin: 20px 0;">
                                    <a href="${unsubscribeUrl}" style="background-color: #dc2626; color: white; padding: 8px 16px; text-decoration: none; border-radius: 4px; font-size: 12px;">Unsubscribe</a>
				...
				try {
					await sendBrevoEmail(
						recipient.email,
						"New Service Added! - Biruh Kids Clinic",
						htmlContent,
						"Biruh Kids Pediatric Speciality Clinic"
					);
					return true;
					...
					console.error(
						`Failed to send department service newsletter to ${recipient.email}:`,
						error
					);
					return false;
				}
		...
		console.log(
			`Department service newsletter sent to ${sentCount}/${uniqueRecipients.length} recipients`
		);
	});
};