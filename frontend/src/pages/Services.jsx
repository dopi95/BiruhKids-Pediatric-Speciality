...
const Services = () => {
  ...
  const translations = {
    En: {
      ...
      heroDesc:
        "Comprehensive pediatric healthcare services across specialized departments",
  ...
  const getGradient = (title) => {
    const key = Object.keys(gradientMap).find((k) =>
      title.toLowerCase().includes(k.toLowerCase())
    );
  ...
  const currentServices =
    selectedDepartment?.services.slice(
      indexOfFirstService,
      indexOfLastService
    ) || [];
  const totalPages = selectedDepartment
    ? Math.ceil(selectedDepartment.services.length / servicesPerPage)
    : 0;
        ...
        <div className="max-w-7xl mx-auto px-4">
                ...
                return (
                  ...
                  >
                    <div className="text-center">
                      <div
                        className={`bg-gradient-to-br ${dept.gradient} w-20 h-20 rounded-full flex items-center justify-center mx-auto mb-6`}
                      >
            ...
            <div className="hidden xl:block relative">
              ...
              <button
                onClick={() =>
                  setDesktopSlide(
                    Math.min(
                      Math.ceil(departments.length / 4) - 1,
                      desktopSlide + 1
                    )
                  )
                }
              ...
              <div className="overflow-hidden mx-12">
                ...
                >
                  {Array.from({
                    length: Math.ceil(departments.length / 4),
                  }).map((_, slideIndex) => (
                    <div
                      key={slideIndex}
                      className="w-full flex-shrink-0 grid grid-cols-4 gap-8"
                    >
                      {departments
              ...
              <div className="flex justify-center mt-6 space-x-3">
                {Array.from({ length: Math.ceil(departments.length / 4) }).map(
                  (_, index) => (
                    <button
                      key={index}
          ...
          <div className="xl:hidden relative">
            ...
            <button
              onClick={() =>
                setCurrentSlide(
                  currentSlide === 0 ? departments.length - 1 : currentSlide - 1
                )
              }
              className="hidden md:block absolute left-4 top-1/2 -translate-y-1/2 z-10 bg-white shadow-lg rounded-full p-3 hover:bg-gray-50 transition-colors duration-200"
            >
              <ChevronLeft className="h-6 w-6 text-gray-600" />
            </button>
            <button
              onClick={() =>
                setCurrentSlide(
                  currentSlide === departments.length - 1 ? 0 : currentSlide + 1
                )
              }
              className="hidden md:block absolute right-4 top-1/2 -translate-y-1/2 z-10 bg-white shadow-lg rounded-full p-3 hover:bg-gray-50 transition-colors duration-200"
            >
              <ChevronRight className="h-6 w-6 text-gray-600" />
            </button>

            <div className="overflow-hidden rounded-2xl shadow-lg md:mx-16">
              <div
                className="flex transition-transform duration-500 ease-in-out"
              ...
              >
                {departments.map((dept) => {
                  ...
                  return (
                    <div
                      key={dept.id}
                      className="w-full flex-shrink-0 bg-white p-6 md:p-8 border-l-4 border-blue-500"
                    >
                      <div className="text-center">
                        <div
                          className={`bg-gradient-to-br ${dept.gradient} w-20 h-20 rounded-full flex items-center justify-center mx-auto mb-6`}
        ...
        <section id="services-section" className="py-20 bg-white">
          <div className="max-w-7xl mx-auto px-4">
            <div className="mb-12 text-center">
              ...
              <div
                className={`h-1 w-24 bg-gradient-to-r ${selectedDepartment.gradient} rounded-full mx-auto`}
              ></div>
            </div>
