			...
			const response = await fetch(
				`${import.meta.env.VITE_API_BASE_URL}/chat`,
				{
					...
					body: JSON.stringify({
						message: userMessage,
						conversationHistory:
							conversationHistory.current.slice(1), // Exclude system prompt
					}),
				}
			);
	...
	const getIntelligentResponse = (question) => {
		...
		for (const [category, words] of Object.entries(keywords)) {
			if (words.some((word) => lowerQuestion.includes(word))) {
				switch (category) {
					case "appointment":
						return "📅 You can book an appointment online through our website by filling out the appointment form. Once submitted, you'll receive a confirmation email or we'll call you to confirm your appointment. For further assistance, contact us at ☎️ 0963555552 / 0939602927 / 0984650912 or biruhkidsclinic@gmail.com.";
					case "services":
					...
					case "emergency":
						return "🚨 Yes! We provide 24/7 pediatric emergency services. For immediate emergencies, please call ☎️ 0963555552 / 0939602927 / 0984650912 right away.";
					case "doctors":
					...
					<div className="bg-gradient-to-r from-blue-500 to-blue-700 text-white flex justify-between items-center p-4">
						<div>
							<h2 className="font-bold text-lg">
								Biruhkids AI Chatbot
							</h2>
							<p className="text-xs opacity-90">
								Welcome to Biruhkids Pediatric Specialty Clinic{" "}
								<br />
						...
						<div className="flex-1 px-4 py-3 space-y-4 bg-gray-50 overflow-y-auto">
							...
							{history.length === 0 && (
								<p className="text-gray-500 text-sm mt-2">
									...
									and it will be saved here when you close the
									chat.
								</p>
							)}
